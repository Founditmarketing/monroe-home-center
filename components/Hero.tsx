    ...
    <div className="relative h-screen min-h-[950px] flex items-center justify-center overflow-hidden bg-black">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-8">
          <button id="cta-unleash" className="group relative px-12 py-7 bg-red-600 hover:bg-red-700 text-white font-black uppercase tracking-[0.3em] text-xl transition-all animate-neon-pulse flex items-center gap-4 overflow-hidden">
          ...
          <button id="cta-view-inventory" className="px-12 py-7 bg-white/5 hover:bg-white/10 backdrop-blur-xl border border-white/20 text-white font-black uppercase tracking-[0.3em] text-xl transition-all flex items-center gap-3 group">
            <Zap size={20} className="text-red-600 group-hover:scale-125 transition-transform" />
            VIEW INVENTORY
          </button>
        </div>

        {/* Floating Trending Cards - "The Sizzle" */}
        <div className="hidden lg:flex absolute -bottom-48 left-1/2 -translate-x-1/2 gap-12 pointer-events-none">
          <div className="glass-morphism p-7 w-80 rotate-[-4deg] animate-float-hero pointer-events-auto cursor-pointer hover:rotate-0 hover:-translate-y-8 transition-all duration-700 border-l-4 border-l-red-600">
            <div className="relative h-44 mb-6 overflow-hidden">
              <img src="https://images.unsplash.com/photo-1512917774080-9991f1c4c750?auto=format&fit=crop&q=80&w=400" className="w-full h-full object-cover scale-110 hover:scale-100 transition-transform duration-[1.5s]" alt="The Jackson" />
              <div className="absolute top-4 left-4 px-4 py-1.5 bg-green-500 text-white text-[10px] font-black uppercase tracking-widest shadow-2xl">Quick Ship</div>
            </div>
            <h3 className="text-white font-black uppercase tracking-tighter text-2xl">The Jackson</h3>
            <div className="flex justify-between items-center mt-5">
              <span className="text-red-600 font-oswald text-3xl font-black italic">$184,900</span>
              <div className="flex items-center gap-1 text-white/30 text-[9px] font-black uppercase tracking-widest">
                <MapPin size={10} /> Monroe, LA
              </div>
            </div>
          </div>

          <div className="glass-morphism p-7 w-80 translate-y-8 rotate-[3deg] animate-float-hero [animation-delay:2s] pointer-events-auto cursor-pointer hover:rotate-0 hover:-translate-y-4 transition-all duration-700 border-l-4 border-l-red-600">
            <div className="relative h-44 mb-6 overflow-hidden">
              <img src="https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&q=80&w=400" className="w-full h-full object-cover scale-110 hover:scale-100 transition-transform duration-[1.5s]" alt="The Royal" />
              <div className="absolute top-4 left-4 px-4 py-1.5 bg-green-500 text-white text-[10px] font-black uppercase tracking-widest shadow-2xl">Verified Stock</div>
            </div>
            <h3 className="text-white font-black uppercase tracking-tighter text-2xl">The Royal</h3>
            <div className="flex justify-between items-center mt-5">
              <span className="text-red-600 font-oswald text-3xl font-black italic">$249,000</span>
              <div className="flex items-center gap-1 text-white/30 text-[9px] font-black uppercase tracking-widest">
                <MapPin size={10} /> Baton Rouge, LA
              </div>
            </div>
          </div>
        </div>
